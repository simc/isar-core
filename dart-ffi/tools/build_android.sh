...
if [ "$1" = "x86" ]; then
  ...
  cargo build --target i686-linux-android --release -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort
  mv "target/i686-linux-android/release/libisar.so" "libisar_android_x86.so"
elif [ "$1" = "x64" ]; then
  rustup target add x86_64-linux-android
  cargo build --target x86_64-linux-android --release -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort
  mv "target/x86_64-linux-android/release/libisar.so" "libisar_android_x64.so"
elif [ "$1" = "armv7" ]; then
  rustup target add armv7-linux-androideabi
  cargo build --target armv7-linux-androideabi --release -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort
